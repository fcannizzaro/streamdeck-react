...
- **Vite bundling** -- config template, `streamDeckReact({ targets })`, production target selection
- **Runtime prerequisites** -- fonts and platform-specific Takumi bindings
...
skills/
  streamdeck-react/
    ...
    references/
      ...
      bundling.md                 # Vite config and native bindings
...
3. When working on a specific topic (e.g., hooks or Vite config), the agent follows links to `references/` for detailed signatures and examples.
