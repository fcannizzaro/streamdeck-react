...
For simple cross-action state sharing without external dependencies, use the built-in [Action Coordinator](/guides/coordinator). Enable it in `createPlugin()` and use `useChannel()` for shared state:
...
| Simple cross-action state (built-in)             | `useChannel()` via [Action Coordinator](/guides/coordinator)        |
| Know which actions are visible                   | `useActionPresence()` via [Action Coordinator](/guides/coordinator) |
