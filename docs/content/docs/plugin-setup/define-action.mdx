...
Component for full-strip rendering across all encoders. When set, replaces per-encoder `dial` display with a single shared React tree that spans the entire Stream Deck+ touch strip. See the [TouchBar Component](/guides/touchbar) guide.
