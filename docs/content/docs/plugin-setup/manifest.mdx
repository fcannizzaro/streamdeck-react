...
1. **Plugin metadata** from the `manifest` option in your Rollup or Vite config.
...
The manifest file is only written when its content has changed. This avoids unnecessary rebuilds when using `rollup -c -w` or `vite build --watch`.
