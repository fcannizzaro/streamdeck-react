...
- `streamDeckReact({ targets: [...] })` copies the Takumi native binaries you want to ship into your output directory.
- In watch mode, `streamDeckReact()` can infer the current supported host target. For production builds, prefer explicit `targets`.
