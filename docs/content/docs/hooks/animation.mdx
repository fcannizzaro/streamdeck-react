...
- The `fps` option (default 60) controls the tick rate. Actual frame rate is still capped by `renderDebounceMs`.
...
Both hooks use `useTick` internally and accept an `fps` option (default 60). The actual frame rate is capped by the render debounce (default 16ms = ~60fps theoretical max). In practice, rendering time limits real throughput to roughly 10-30fps depending on component complexity.
