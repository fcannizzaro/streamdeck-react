...
interface RenderConfig {
  ...
  touchstripImageFormat: OutputFormat; // Default: "webp"
...
| `touchstripImageFormat`   | `touchstripImageFormat`   | `"webp"`                        |
...
At 60fps touchstrip rendering, each frame allocates ~320KB of raw RGBA buffers (800×100×4 bytes). Without pooling, V8's garbage collector must reclaim ~20MB/s of short-lived buffers, causing periodic frame drops visible as stutter.
...
**Why**: Takumi's native rasterization (Rust via NAPI) blocks the calling thread for 5–30ms per frame. During 60fps touchstrip animation, this leaves almost no time for processing `keyDown` or `dialRotate` events on the main thread.
...
The renderer writes PNG by default and can be configured to output WebP via `imageFormat: "webp"` in `createPlugin()`. For touchstrip segments specifically, `touchstripImageFormat` defaults to `"webp"` because Takumi encodes WebP natively during the render call itself, avoiding the separate PNG encoding step.
