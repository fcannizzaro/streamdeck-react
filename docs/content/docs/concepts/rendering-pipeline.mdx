...
If the key is found in the byte-bounded LRU image cache, the cached data URI (or raw buffer for touchbar) is returned immediately — skipping the entire Takumi render.
...
| `renderToRaw`     | Touchbar    | Raw RGBA `Buffer`      | Touchbar cache (8 MB default) |

Both follow the same 4-phase skip hierarchy. The key difference is that `renderToRaw` skips the base64 encoding step — the caller (TouchBarRoot) crops per-encoder segments from the raw buffer and encodes each independently.
...
interface RenderConfig {
  ...
  touchbarCacheMaxBytes: number; // Default: 8 MB
  renderPool: RenderPool | null;
  touchbarImageFormat: OutputFormat; // Default: "webp"
...
| `touchbarCacheMaxBytes` | `touchbarCacheMaxBytes` | `8388608` (8 MB)                |
| `useWorker`             | `renderPool`            | `true` (creates a `RenderPool`) |
| `touchbarImageFormat`   | `touchbarImageFormat`   | `"webp"`                        |

## Image Cache

The image cache is a byte-bounded LRU cache shared across all React roots. Unlike a count-bounded cache, byte-bounding handles the wide size variance of Stream Deck images correctly — a 72×72 key PNG is ~2KB while an 800×100 raw RGBA touchbar buffer is 320KB.

Two separate caches exist:

- **Image cache** (`ImageCache<string>`) — stores base64 data URIs for key/dial renders. Default budget: **16 MB**.
- **Touchbar cache** (`ImageCache<Buffer>`) — stores raw RGBA buffers. Default budget: **8 MB**.
...
Cache sizes are configurable via `imageCacheMaxBytes` and `touchbarCacheMaxBytes` in `createPlugin()`. Set to `0` to disable.

## Buffer Pool

At 60fps touchbar rendering, each frame allocates ~320KB of raw RGBA buffers (800×100×4 bytes). Without pooling, V8's garbage collector must reclaim ~20MB/s of short-lived buffers, causing periodic frame drops visible as stutter.
...
**Why**: Takumi's native rasterization (Rust via NAPI) blocks the calling thread for 5–30ms per frame. During 60fps touchbar animation, this leaves almost no time for processing `keyDown` or `dialRotate` events on the main thread.
...
interface RenderMetrics {
  ...
  touchbarCacheBytes: number; // TouchBar cache memory usage
}
```
...
The renderer writes PNG by default and can be configured to output WebP via `imageFormat: "webp"` in `createPlugin()`. For touchbar segments specifically, `touchbarImageFormat` defaults to `"webp"` because Takumi encodes WebP natively during the render call itself, avoiding the separate PNG encoding step.
