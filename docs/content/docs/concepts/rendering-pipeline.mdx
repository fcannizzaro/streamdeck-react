...
interface RenderConfig {
  renderer: Renderer; // Shared Takumi renderer instance
  imageFormat: OutputFormat; // "png" | "webp"
  caching: boolean; // FNV-1a hash deduplication
  devicePixelRatio: number; // Default: 1
}
```

Dimensions (width/height) are determined automatically from the device type and controller, and passed to the render function separately. See [Device Sizes](/reference/device-sizes) for the full table.
